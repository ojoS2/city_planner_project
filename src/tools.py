...
class UnidirectionalOpenSegment():
    def __init__(self, holding_volume, exiting_flux, init_position,
                 end_position, capacity, penalidade, id) -> None:
    ...
    def __str__(self):
        descricao = f"""
_Cano unidirecional com reservatório objeto_
Volume a entrar : {self.holding_volume}
Volume saindo : {self.exiting_flux}
capacidade do cano : {self.capacity}
fluxo de saida : {self.exiting_flux}
...
"""
    ...
    def fluxo_saida_instantaneo(self):
        if self.holding_volume < self.capacity:
            self.exiting_flux = self.holding_volume
            self.holding_volume = 0
            return self
        else :
            self.holding_volume = self.holding_volume - self.capacity
            self.exiting_flux = self.capacity
            return self

    def entering_reservoir_from_int_src(self, node_list):
        aux = 0
        for i in node_list.exiting_flux:
            self.holding_volume = self.holding_volume + i
            aux = aux + i
        return aux
...
def teste_UOS():
    teste = UnidirectionalOpenSegment(holding_volume = 43, exiting_flux = 0,
                        init_position = (0, 0), end_position = (1, 0),
                        capacity = 10, penalidade = 0, id = 0)
    print(teste)
    print(teste.fluxo_saida_instantaneo())
    print(teste.fluxo_saida_instantaneo())
    print(teste.fluxo_saida_instantaneo())
    print(teste.fluxo_saida_instantaneo())
    print(teste.fluxo_saida_instantaneo())
...
def initialize_BOS_list(params_list):
    ...
    for params in params_list:
        OL.append(initialize_BOS(left_holding_volume = params[0], right_holding_volume = params[1],
...
def initialize_BOS_rectangular_network(inf_lim_x, sup_lim_x, inf_lim_y, sup_lim_y,
    ...
    node_list = build_retangle_grid (inf_lim_x, sup_lim_x, inf_lim_y, sup_lim_y,
                                     entry_points_coor, exit_points_coor)
    ...
    for _, row in node_list.iterrows():    
        if row["type"] == "center":
            ...
            if row["x"] == inf_lim_x:
                ...
                return None
    pipes_list = initialize_BOS_list(params_list)
...
def network_flux_experiment(loading_time, simulation_time, inf_lim_x, sup_lim_x, inf_lim_y, sup_lim_y,
                                       entry_points_coor, exit_points_coor):
    struct, objects_list, node_list = initialize_BOS_rectangular_network(inf_lim_x, sup_lim_x, inf_lim_y, sup_lim_y,
                                       entry_points_coor, exit_points_coor)
        ...
        if row['type'] == 'center':
            ...
            if row['x'] == sup_lim_x:
                ...
                if row['y'] == sup_lim_y:
                    ...
                    nodes_df.loc[idx, 'nn'] = 2
    if visualization:
        plot_rectangular_grid(nodes_df)
    return nodes_df
...
network_flux_experiment(loading_time = 5, simulation_time = 50, inf_lim_x = 0,
